...
def read_squad_examples(context, question_text):
...
def write_predictions(all_examples, all_features,
    ...
    best_results = all_nbest_json[list(dict(all_nbest_json).keys())[0]]
    cleaned = [dict(res) for res in best_results]
    return cleaned


def process_all(model, tokenizer, context, question):
...
if __name__ == '__main__':
    ...
    """
    context2 = """
    The most sophisticated form of HIE creates semantic interoperability, thereby 
    bridging the many ways the same concepts can be expressed by providers and represented in EHRs 
    and other clinical systems. 
    This involves a complex process that maps external 
    representations of clinical concepts to some standard 
    developed or adopted by the HIE. 
    The premier example of this here in the U.S. is the Indiana Health Information Exchange 
    or IHIE. It says it is the largest such organization in the country. 
    ...
    These include the reporting of laboratory test results we discussed earlier and of the quality 
    metrics that are increasingly required for providers under the new payment models we discussed in lesson one. 
    The Regenstrief Institute in Indiana created the expensive and sophisticated technology used by IHIE.
    """
    with open('data/health_today.txt', 'r') as f:
        content = f.readlines()
        content = ''.join(content).replace('\n', ' ')
    while 1:
        search_question = input("Ask a question: ")
        print(search_question)
        x = process_all(model, tokenizer, content, search_question)
        clean_answers = [{'text': b['text'], 'probability': b['probability']} for b in x]
        print(clean_answers)

    #question = 'What created the expensive and sophisticated technology?'
    #x = process_all(model, tokenizer, context2, search_question)
